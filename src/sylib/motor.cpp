...
namespace sylib {
    SylviesPogVelocityEstimator::SylviesPogVelocityEstimator(double motorGearing) : smaFilterVelocity(3), smaFilterAccel(15),
                                 smaFilterJerk(3), smaFilterSnap(3), medianFilter(7,2,1),
                                 emaFilter(), motorGearing(motorGearing), preFilterAccelSolver(), preFilterJerkSolver(), preFilterSnapSolver(), outputAccelSolver(),
                                 outputJerkSolver(), outputSnapSolver(),maxFilterAccel(20){
    ...
    std::int32_t Motor::get_estimator_voltage() const{mutex_lock _lock(sylib_port_mutexes[smart_port-1]);return motorVoltageEstimator.getOutput();}
    std::int32_t Motor::get_tbh_voltage() const{mutex_lock _lock(sylib_port_mutexes[smart_port-1]);return motorTBHController.getOutput();}
